...
.PHONY: zig-build
zig-build:
	zig build -fsummary -Dlog -Dchance -Dcalc -p build
	zig build -fsummary -Dshowdown -Dlog -Dchance -Dcalc -p build
...
.PHONY: zig-test
zig-test:
	zig build -fsummary -Dlog -Dchance -Dcalc test
	zig build -fsummary -Dshowdown -Dlog -Dchance -Dcalc test
...
zig-coverage:
	...
	zig build -fsummary -Dshowdown -Dlog -Dchance -Dcalc test -Dtest-coverage=coverage/zig/pkmn-showdown
...
.PHONY: t
t:
	zig build -fsummary test -Dlog -Dchance -Dcalc -Dtest-file=src/lib/gen$(gen)/test.zig -Dtest-filter="$(filter)" $(opt)
